...
if (-not (Get-Command tailwindcss -ErrorAction SilentlyContinue)) {
	...
	Add-PodeFileWatcher -Path $ScriptBase -Exclude "$ScriptBase/public" -ScriptBlock {
		tailwindcss --input $ScriptBase/tailwind/style.tw.css --output $ScriptBase/public/style.css
	}
}
